...
| ID | 状態 | 内容 | 依存 | 完了条件 |
|---|---|---|---|---|
| M06-T01 | Backlog | 製品品質基準、対象環境、性能予算を定義する | M05 | 各品質項目に検証可能な合否条件がある |
| M06-T02 | Backlog | 表示・音量・演出・入力設定と永続化を実装する | T01 | 再起動後も設定が保持され、安全な既定値へ戻せる |
| M06-T03 | Backlog | 音、画面遷移、カード操作、結果演出を仕上げる | T02 | 主要操作の状態変化を視覚と音で識別できる |
...
外部素材や対応環境へ費用・契約が生じる判断だけは、該当タスクでユーザー確認を行う。
