...
| ID | 状態 | 内容 | 依存 | 完了条件 |
|---|---|---|---|---|
| M01-T01 | Ready | 構造化表示のAPI・データモデル設計を`docs/design/`へ追加する | なし | 公開境界、拡張方法、互換性、Action対応がレビュー可能な粒度で定義されている |
| M01-T02 | Backlog | 共通の読み取り専用モデルと段階導入用インターフェースをRuntimeへ追加する | T01 | Unity非依存でコンパイルでき、既存`IGame`実装を破壊しない |
| M01-T03 | Backlog | Crazy Eightsへ構造化状態と操作記述を実装する | T02 | viewer別状態と全合法手を文字列解析なしで取得できる |
| M01-T04 | Backlog | .NET契約テストを追加する | T03 | 非公開手札、山札順、指定suit、draw/play境界、Action一対一対応を固定する |
| M01-T05 | Backlog | Unity Edit Modeの同等テストと利用例を追加する | T04 | Unity Standard対象で同じ契約を検証し、READMEに利用例がある |
| M01-T06 | Backlog | 必須検証と互換性確認を行いM01を完了する | T05 | 下記の完了条件をすべて満たし、ロードマップを更新する |
